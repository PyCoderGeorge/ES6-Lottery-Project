	    ...
	    _createClass(Timer, [{
	        ...
	        value: function countDown(end, update, handle) {
	            ...
	            if (now - end) {
	                ...
	                var px_m = 1000 * 60; // 一分钟多少毫秒
	                var px_s = 1000; // 一秒多少毫秒

	                var d = Math.floor(last_tiem / px_d); // 剩余多少天
	                var h = Math.floor((last_tiem - d * px_d) / px_h); // 剩余多少小时
	                var m = Math.floor((last_tiem - d * px_d - h * px_h) / px_m); // 剩余多少分钟
	                var s = Math.floor((last_tiem - d * px_d - h * px_h - m * px_m) / px_s); // 剩余多少毫秒
